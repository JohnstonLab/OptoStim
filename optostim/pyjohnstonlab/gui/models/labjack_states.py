...
from pyjohnstonlab.gui.models.labjack_state_model import LabJackStateModel

log = logging.getLogger(__name__)


class LabJackStatesTableModel(QAbstractTableModel):

    mime_format = 'application/x-qabstractitemmodeldatalist'

    def __init__(self, labjack, parent=None):
    ...
    def columnCount(self, parent=None, *args, **kwargs):
        return len(self.labjack.labels())

    def data(self, model_index, role=None):
        if role == Qt.DisplayRole and len(self.states) > 0:
    ...
    def headerData(self, p_int, Qt_Orientation, role=None):
        if role == Qt.DisplayRole:
            if Qt_Orientation == Qt.Horizontal:
                return self.labjack.label(p_int)
            return p_int
        return QVariant()
