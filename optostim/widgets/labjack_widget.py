...
class LabJackWidget(QWidget):
    ...
    def on_executeStatesButton_pressed(self):
        if self.is_executing:
            ...
            self.thread = QThread()
            self.worker = ExecuteStatesWorker(number_of_times=count, delay=delay,
                                              device=self.labjack_device, states=self.model.states)
    ...
    def on_deleteStates_pressed(self):
        ...
        [self.model.removeRow(model_index.row() - i) for i, model_index in enumerate(model_indices)]
